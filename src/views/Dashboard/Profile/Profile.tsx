               ...
               <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-6 lg:gap-8">
                  ...
                  <div className="order-1">
                     <div className="bg-white rounded-xl p-6 sm:p-8 shadow-md text-center lg:sticky lg:top-42">
                        ...
                        <div className="relative w-32 h-32 sm:w-40 sm:h-40 mx-auto mb-6">
                           <div className="w-full h-full rounded-full bg-[#38B6FF] flex items-center justify-center overflow-hidden border-4 border-[#FFFDFE]">{user.avatar ? <Image src={user.avatar} alt={user.username} fill className="object-cover" /> : <span className="text-5xl sm:text-6xl font-bold text-white">{user.username.charAt(0).toUpperCase()}</span>}</div>
                           {isEditing && (
                  ...
                  <div className="flex flex-col gap-6 order-2">
                     ...
                     <div className="bg-white rounded-xl p-6 sm:p-8 shadow-md">
                        ...
                        <div className="grid gap-6">
                           ...
                           <div>
                              ...
                              {isEditing ? <input type="text" value={editedUser.username} onChange={(e) => setEditedUser({ ...editedUser, username: e.target.value })} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-[#38B6FF] rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-[#38B6FF] focus:ring-opacity-50" /> : <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-[#FFFDFE] rounded-lg text-gray-600 text-base">{user.username}</div>}
                           </div>
                           ...
                           <div>
                              ...
                              {isEditing ? <input type="email" value={editedUser.email} onChange={(e) => setEditedUser({ ...editedUser, email: e.target.value })} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-[#38B6FF] rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-[#38B6FF] focus:ring-opacity-50" /> : <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-[#FFFDFE] rounded-lg text-gray-600 text-base">{user.email}</div>}
                           </div>

                           {/* Phone */}
                           <div>
                              <label className="flex items-center gap-2 mb-2 text-black font-semibold text-sm sm:text-base">Phone Number</label>
                              {isEditing ? <input type="tel" value={editedUser.phone || ""} onChange={(e) => setEditedUser({ ...editedUser, phone: e.target.value })} placeholder="Enter phone number" className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-[#38B6FF] rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-[#38B6FF] focus:ring-opacity-50" /> : <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-[#FFFDFE] rounded-lg text-gray-600 text-base">{user.phone || "Not provided"}</div>}
                           </div>

                           {/* Address */}
                           <div>
                              <label className="flex items-center gap-2 mb-2 text-black font-semibold text-sm sm:text-base">Address</label>
                              {isEditing ? <input type="text" value={editedUser.address || ""} onChange={(e) => setEditedUser({ ...editedUser, address: e.target.value })} placeholder="Enter address" className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-[#38B6FF] rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-[#38B6FF] focus:ring-opacity-50" /> : <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-[#FFFDFE] rounded-lg text-gray-600 text-base">{user.address || "Not provided"}</div>}
                           </div>
                        </div>
                        ...
                        {isEditing && (
                           <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 pt-6 border-t border-gray-200">
                              <button onClick={handleCancel} className="flex items-center justify-center gap-2 px-4 py-3 bg-white text-gray-600 border-2 border-gray-400 rounded-lg font-semibold hover:bg-gray-50 transition-colors">
                                 <X className="w-5 h-5" />
                                 Cancel
                              </button>
                              <button onClick={handleSave} className="flex items-center justify-center gap-2 px-4 py-3 bg-[#38B6FF] text-white rounded-lg font-semibold hover:bg-[#2A9FE8] transition-colors">
                     ...
                     <div className="bg-white rounded-xl p-6 sm:p-8 shadow-md">
                        ...
                        {isEditing ? <textarea value={editedUser.bio || ""} onChange={(e) => setEditedUser({ ...editedUser, bio: e.target.value })} placeholder="Tell us about yourself..." rows={4} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-[#38B6FF] rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-[#38B6FF] focus:ring-opacity-50 resize-y" /> : <p className="text-gray-600 leading-relaxed text-base">{user.bio || "No bio added yet."}</p>}
                     </div>
