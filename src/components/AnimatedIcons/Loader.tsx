...
const LoaderPinwheelIcon = forwardRef<LoaderPinwheelIconHandle, LoaderPinwheelIconProps>(({ isLoading = false, onMouseEnter, onMouseLeave, className, size = 28, ...props }, ref) => {
  ...
  // 🔁 control animation via isLoading
  useEffect(() => {
    if (isLoading) {
  ...
  // hover (disabled if isLoading is true)
  const handleMouseEnter = useCallback(
    ...
    [controls, onMouseEnter, isLoading]
  );
  ...
  const handleMouseLeave = useCallback(
    ...
    [controls, onMouseLeave, isLoading]
  );
